     ...
     api.registerTool(
          {
               name: 'memvid_forget',
               description:
                    'Delete specific memories from archives. GDPR-compliant.',
               label: 'Forget Memory',
               parameters: Type.Object({
                    ...
                    archiveName: Type.Optional(
                         Type.String({ description: 'Archive to search' })
                    ),
                    frameId: Type.Optional(
                         Type.String({ description: 'Specific frame ID' })
                    ),
               }),
                         ...
                         if (frameId && archiveName) {
                              ...
                              if (results.length === 1 && results[0].frameId) {
                                   ...
                                   const targetPath = join(
                                        memvidClient.getArchivesDir(),
                                        result.archiveName
                                   );
                                   await memvidClient.delete(
                                        targetPath,
                                        result.frameId as string
                                   );
