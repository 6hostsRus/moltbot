          ...
          if (mergedCfg.autoRecall) {
               api.on('before_agent_start', async (event) => {
                    ...
                    try {
                         ...
                         api.logger.warn?.(
                              `memory-memvid: recall failed: ${String(err)}`
                         );
                    }
               });
          }
               ...
               async (event) => {
                    if (mergedCfg.autoCapture) {
                         ...
                         try {
                              ...
                              api.logger.warn?.(
                                   `memory-memvid: capture failed: ${String(err)}`
                              );
                         }
                    }
                    if (mergedCfg.autoArchiveSessions) {
                         ...
                         try {
                              ...
                              const thresholdMB =
                                   mergedCfg.sessionArchiveThresholdMB || 10;
          ...
          api.registerService({
               ...
               start: async () => {
                    try {
                         ...
                         api.logger.error(
                              `memory-memvid: CLI not found at ${memvidClient.getCliPath()}`
                         );
                         throw err;
                    }
               },
