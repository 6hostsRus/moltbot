...
import { mergeConfig, DEFAULT_CONFIG } from './src/types/types.js';
...
describe('memory-memvid plugin', () => {
     ...
     it('should have valid config schema', () => {
          ...
          expect(memvidPlugin.configSchema.type).toBe('object');
     });
     ...
     describe('config', () => {
          ...
          it('should use default values when user config is empty', () => {
               ...
               expect(merged.archivesDir).toBe(DEFAULT_CONFIG.archivesDir);
               expect(merged.enrichmentEngine).toBe(
                    DEFAULT_CONFIG.enrichmentEngine
               );
     ...
     describe('lifecycle hooks', () => {
          it('should register lifecycle hooks when enabled', () => {
               ...
               expect(
                    hooks.filter((h) => h === 'agent_end').length
               ).toBeGreaterThanOrEqual(2); // auto-capture + auto-archive
          });
